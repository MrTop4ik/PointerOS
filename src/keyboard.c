...
void irq1_handler(struct InterruptRegisters *regs){
    ...
    switch(scanCode){
        ...
        case 0x48:
            ...
            break;
        case 0x50:
            if (!press && term_row < VGA_HEIGHT -1){
                term_row++;
                term_update_cursor();
            }
            break;
        ...
        case 0x48 - 1:
            term_col = 0; 
            while (term_col <= VGA_WIDTH - 1){
                uint16_t char_at = vga_buffer[term_row * VGA_WIDTH + term_col];
                if ((char_at & 0xFF) != ' '){
                    break;
                }
                term_col++;
        ...
        default:
            if (press == 0){
                if (capsLock && capsOn){
                    term_putchar(lowercase[scanCode]);
                } else if (capsLock || capsOn){
                    term_putchar(uppercase[scanCode]);
                } else {
                    term_putchar(lowercase[scanCode]);
                }
